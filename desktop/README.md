...
내장 샘플은 행정안전부 `인공지능정부실`과 `참여혁신조직실`을 A4 세로 왼쪽면에 배치합니다. 0.3부터는 같은 스키마의 `*.native.json`을 앱에서 선택하거나 A4 미리보기 영역으로 끌어놓아 다른 조직도도 생성할 수 있습니다.
...
4. 필요하면 외부 `*.native.json`을 불러와 사전검사
5. 사용자가 생성 버튼을 누르면 네이티브 HWPX 생성·재열기 검증
