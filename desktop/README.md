...
0.5부터는 로컬 이력 DB에서 3~4개 기준일을 고른 뒤 **애니메이션 영상 저장**을 누를 수 있습니다. A3 용지는 화면에 고정하고 각 시점의 조직도 본체를 먼저 왼쪽부터 완성한 뒤, 시점 간 대응 점선을 `1→2`, `2→3`, `3→4` 순서로 연결합니다. 최신 WebView2에서는 H.264 MP4를 우선 사용하고, MP4 인코더가 없는 PC에서는 WebM으로 자동 대체합니다.

0.6부터는 WebView2가 지원할 때 Canvas 프레임을 `requestFrame()`으로 정확히 30fps 단위로 요청합니다. 구형 WebView2는 기존 자동 캡처로 안전하게 폴백합니다. 영상·원본 명세·메타데이터는 임시 파일에 먼저 기록하고 영상 바이트를 다시 읽어 SHA-256을 대조한 뒤 세 파일을 한 묶음으로 확정하므로, 저장 중 오류가 나면 불완전한 결과를 남기지 않습니다.
...
오류가 있으면 생성을 막고, 끊긴 접합이나 대각선처럼 확인이 필요한 항목은 경고로 표시합니다. 저장한 HWPX를 Automation으로 다시 열어 `선택한 용지 1쪽`과 `네이티브 객체 수 일치`를 확인한 경우에만 앱 화면에 검증 통과를 표시합니다.
...
- `이름.mp4` 또는 `이름.webm`: A3 고정 조직개편 애니메이션
- `이름.video.native.json`: 영상에 사용한 원본 네이티브 명세
- `이름.video.json`: 해상도·fps·프레임 수·캡처 방식·구간·코덱·사전검사·SHA-256 결과

## 3~4단 조직개편 영상 만들기

1. 직제·시행규칙을 입력하거나 공식 API에서 기준일 조직을 불러옵니다.
2. `현재 조직도 이력 저장`으로 필요한 기준일을 로컬 이력 DB에 저장합니다.
3. 1열부터 4열까지 기준일을 고르고 `대비 조직도 보기`를 누릅니다.
4. A3 대비표의 문구·상자·계선을 확인한 뒤 `애니메이션 영상 저장`을 누릅니다. 버튼에는 이 PC에서 실제 사용할 `MP4` 또는 `WEBM` 형식이 표시됩니다.
5. 3단은 324프레임·약 10.8초, 4단은 420프레임·14초의 작도 스케줄로 앱 안에서 녹화됩니다. 진행창에는 현재 요청 프레임과 프레임 고정/호환 녹화 상태가 표시됩니다.
6. 저장 후 영상은 SHA-256으로 다시 검증됩니다. 결과는 `%USERPROFILE%\Documents\직제기구도`에 원본 명세·메타데이터와 함께 저장되고 기본 동영상 앱으로 열립니다.

영상 생성은 WebView2의 Canvas `captureStream()`·`requestFrame()`과 `MediaRecorder`를 사용하므로 Node.js·Python·FFmpeg를 별도로 설치하지 않습니다. `requestFrame()`이 없으면 자동 캡처로, H.264 MP4 인코더가 없으면 WebM으로 각각 폴백합니다.
