...
시행규칙 분장사무 안에 `○○정책관 내 다른 과의 주관`, `○○교섭관 내 다른 과의 주관`, `○○정책관이 보좌하는 사항 중에서 다른 과의 주관`처럼 보좌기관명이 직접 적힌 경우는 자동으로 소관관계로 저장합니다. `○○정책관은 직제 제n호부터 제m호까지의 사항을 보좌한다`와 각 과의 분장 조문이 같은 직제 호 번호를 직접 재인용하는 경우도 단일 보좌기관 범위에 들어갈 때만 자동 확정합니다. 범위가 겹치거나 과 조문이 호 번호를 재인용하지 않으면 감사 후보로 남겨 사람이 확인합니다.
