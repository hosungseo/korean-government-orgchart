...
- `이름.video.json`: 해상도·fps·구간·코덱·사전검사 결과
...
5. 3단은 약 10.8초, 4단은 14초 동안 앱 안에서 실시간 녹화됩니다. 결과는 `%USERPROFILE%\Documents\직제기구도`에 저장되고 기본 동영상 앱으로 열립니다.

영상 생성은 WebView2의 Canvas `captureStream()`과 `MediaRecorder`를 사용하므로 Node.js·Python·FFmpeg를 별도로 설치하지 않습니다. WebView2가 H.264 MP4를 지원하지 않는 경우 버튼과 결과 파일 확장자가 `WEBM`으로 바뀝니다.
