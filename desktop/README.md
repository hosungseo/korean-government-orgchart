...
가장 안전한 방식은 인터넷이 가능한 수집 PC에서 법제처 기준일 원문과 별표를 미리 내려받아 `--source-dir`에 보관한 뒤, 그 폴더의 `.txt`·`.annexes.json`·조직도 JSON을 승인된 반입 매체로 폐쇄망에 전달하는 것입니다. 폐쇄망 데스크톱은 직제 문언 또는 JSON 모드만 사용하므로 생성 시 외부 URL을 호출하지 않습니다. API 모드는 내부 프록시나 법제처 API 미러가 실제로 접근 가능한 경우에만 사용합니다.
