...
- `catalog`는 연결선을 없애더라도 평면 목록으로 만들지 않습니다. 즉시 상위 조직마다 카드 묶음과 `상위:` 캡션을 만들어 법정 계층을 읽을 수 있게 합니다. 카드 묶음의 그룹 박스도 실제 도면 요소로 취급해 프레임 밖으로 나가거나 서로 겹치면 hard issue로 잡습니다.
