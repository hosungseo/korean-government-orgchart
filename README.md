...
| 입력 | 해석 | 결과 |
| --- | --- | --- |
| 직제·시행규칙 원문 또는 법제처 기준일 연혁 | 설치 문형, 기관 유형, 소관관계, 보직·한시 표식 | **HWPX**(HOP·한글 회람) · **PPTX**(편집) · **SVG**(검토·웹) · **HTML**(인쇄) · **JSON**(재사용) · **MP4/WebM**(Windows 비교 애니메이션) |
...
조직도 상자·문자·계선을 한글에서 각각 선택해 편집해야 할 때는 [`desktop/`](desktop/)의 Windows 앱을 사용합니다. **0.4부터는 앱 안에 직제와 직제 시행규칙 원문을 바로 붙여넣어** 조직을 추출하고 A4 조직도 미리보기를 만든 뒤, 선택한 쪽을 완전 편집형 HWPX로 생성할 수 있습니다. 특정 실·국 여러 개를 쉼표로 지정하면 한 장으로 묶고, 전체 조직처럼 정보량이 많으면 읽을 수 있는 크기를 유지하도록 여러 A4 명세로 자동 분할합니다.

**0.5부터는 3~4개 기준일로 만든 A3 가로 대비표를 Windows 앱 안에서 MP4/WebM 애니메이션으로 저장할 수 있습니다.** A3 화면은 고정하고 네 조직도 본체를 왼쪽부터 먼저 완성한 뒤, 시점 간 대응 점선을 `1→2`, `2→3`, `3→4` 순서로 연결합니다. 최신 WebView2에서는 H.264 MP4를 우선하고 지원 여부에 따라 WebM으로 자동 대체하며, 실행 PC에 Node.js·Python·FFmpeg는 필요하지 않습니다.

**0.6은 30fps 프레임 고정과 저장 무결성을 보강합니다.** 지원되는 WebView2에서는 4단 420프레임(14초), 3단 324프레임(10.8초)의 작도 스케줄을 수동 프레임 요청으로 캡처하고, 구형 런타임은 자동 캡처로 폴백합니다. 영상·원본 명세·메타데이터는 임시 파일에서 SHA-256 재읽기 검증을 통과한 뒤 세 파일을 한 묶음으로 확정합니다.
