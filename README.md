...
| 입력 | 해석 | 결과 |
| --- | --- | --- |
| 직제·시행규칙 원문 또는 법제처 기준일 연혁 | 설치 문형, 기관 유형, 조·항·호·목별 기능, 소관관계, 보직·한시 표식 | **HWPX**(Windows 한글 완전 편집) · **PPTX**(편집) · **SVG**(검토·웹) · **HTML**(인쇄) · **JSON**(근거 재사용) |
...
<details>
<summary>부가 자료: 4단 조직개편 모션 예시</summary>
...
조직도 상자·문자·계선을 한글에서 각각 선택해 편집해야 할 때는 [`desktop/`](desktop/)의 Windows 앱을 사용합니다. **0.7의 중심 기능은 직제·시행규칙 원문을 정확히 읽어 완전 편집형 HWPX로 내보내는 것**입니다.

- 직제와 시행규칙의 `조 → 항 → 호 → 목` 주소, 원문, 소유 조직, 출처를 기능 팩트로 보존합니다.
- `1의2` 같은 가지번호, 여러 줄에 걸친 호, `가목·나목`, 삭제호와 잔여호를 구분합니다.
- 같은 이름의 과가 다음 시점에 남아 있으면 존속을 최우선하고, 사라진 경우에만 개정문·각 호 기능 승계 근거로 점선을 만듭니다.
- 기능 점선에는 양쪽 조문 주소, 일치 기능 수, 포괄률, 신뢰도와 자동·검토 상태를 저장합니다. 이름 유사성만으로 자동 연결하지 않습니다.
- 시행규칙으로 소관 과가 확인된 정책관·기획관은 도표에서 국처럼 하위 과를 묶되, 법적 유형은 보좌기관으로 유지합니다.
- 조밀한 A3 3~4단도 모든 상자와 선이 용지 안에 들도록 대역별로 자동 맞춤합니다.

기존 `kr.go.mois.orgchart.hwp-native/v1` 형식의 `*.native.json` 불러오기도 그대로 지원합니다. 문언 자동변환과 외부 명세 모두 생성 전에 A4/A3 경계·객체 ID·서식·객체 수·자식 계선 접합을 검사합니다. 생성 뒤에는 한글로 HWPX를 다시 열어 쪽수와 객체 수를 대조하며, 재현용 명세와 검증 리포트를 HWPX 옆에 함께 저장합니다. 원문은 로컬에서만 파싱하고 명세나 HWPX에 통째로 저장하지 않습니다.

Windows 저장은 임시 HWPX 생성 → 한글 재열기 → 쪽수·객체 수 확인 → SHA-256 재읽기 검증을 모두 통과한 뒤에만 최종 파일을 확정합니다. `이름.hwpx`, `이름.native.json`, `이름.verification.json`, `이름.legal-evidence.json` 네 파일을 한 묶음으로 원자 저장하며, 중간 실패 시 불완전한 파일을 회수합니다.
...
- 감사·HTML 구조 요약은 소속기관 조상 깊이를 따라 `1차·2차·3차(4차 이상은 4+)`로도 집계합니다. 본부 계선은 소속기관 단계에 섞지 않습니다.
- `catalog`는 연결선을 없애더라도 평면 목록으로 만들지 않습니다. 즉시 상위 조직마다 카드 묶음과 `상위:` 캡션을 만들어 법정 계층을 읽을 수 있게 합니다. 큰 카드 묶음은 용지별 수용량을 넘기 전에 자동 분할하고, 카드 묶음의 그룹 박스도 실제 도면 요소로 취급해 프레임 밖으로 나가거나 서로 겹치면 hard issue로 잡습니다.
- `two-column`은 A4 반쪽처럼 폭이 좁은 면에서는 자식 폭을 계속 쪼개지 않고 좌우 레인 목록으로 전환합니다. 법정 부모-자식 edge는 JSON·trace에 보존하되, 화면에는 각 레인의 수직 레일과 상자 접점선을 가볍게 그려 위→아래 계층을 읽게 합니다. 긴 부모-자식 elbow를 직접 그리지 않아 세로선 끊김·교차·leaf 상자 충돌을 줄이면서도 도표가 빈 목록처럼 보이지 않게 합니다.
- `matrix`는 관·국–과 표형입니다. 반쪽 A4처럼 열 폭이 좁은 출력에서는 조밀한 내부 연결선이 글자와 겹치거나 끊어진 것처럼 보이므로, 법정 관계는 JSON·감사 리포트에 유지하고 화면에는 열 내부의 위→아래 순서로 하위조직을 표시합니다. 한 관·국 밑 leaf 목록이 길면 용지 높이 기준으로 먼저 열을 접고, 한 면의 열 수까지 넘을 때만 페이지를 나눕니다.
...
- A4 반쪽 세로 스택은 과·팀 세로 상자 폭이 18pt 미만으로 떨어지기 전에 가용 폭 기준으로 페이지를 먼저 나눕니다. 따라서 검토서용 반쪽면에서 과명 한 글자 폭이 과도하게 눌린 도면보다 2쪽 분할 또는 카드형 후보가 우선됩니다.
...
- 모든 레이아웃은 인쇄 프레임의 넘침·상자 겹침·너무 짧거나 역방향인 연결선·상자 접점에서 떨어진 route endpoint를 `layout.diagnostics.totalIssues`로 검사합니다. 별도로 자식 상자 간격 불균일, 부모 중심축 어긋남, 연결선 교차, 다른 상자 뒤를 지나가 끊겨 보이는 선-상자 관통, 직접 경로보다 과도하게 긴 선 우회, 카드형 컬럼 불균형, 과도하게 좁은 세로글자 상자는 `qualityIssues`로 집계해 best-fit 점수와 감사 리포트의 `품질` 열에 반영합니다. 감사 리포트는 `--max-nodes`, `--focus`, `--layout catalog`, `--paper a4-landscape` 같은 보정 방향을 제안하고, PPTX/SVG에서도 문제가 발견되면 하단에 경고가 나타납니다. PPTX 하단에는 선택 작도 유형(best-fit 포함), 노드 수, 관계선 수, hard/polish 진단을 함께 인쇄해 검토자가 파일만 열어도 배치 판단을 확인할 수 있습니다.
...
src/evidence.mjs      입력 자료·관계 근거·소관법령·별표 점검 요약
