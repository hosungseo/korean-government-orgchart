...
test("batch-build 출력 형식 별칭을 해석한다", () => {
  ...
  assert.deepEqual(parseOutputFormats("all"), ["svg", "html", "hwpx", "json", "audit", "trace", "pptx"]);
...
`,
  ...
  const result = await runBatchBuild({
    ...
    outputs: "svg,html,hwpx,json,audit,trace",
  });
  ...
  assert.ok((await stat(result.cases[0].outputs.hwpx)).size > 10_000);
  assert.equal((await readFile(result.cases[0].outputs.hwpx)).subarray(0, 2).toString(), "PK");
